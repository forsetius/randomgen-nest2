...
I implemented two sides of the system: the backoffice and the handling of the parking machine API. The idea behind working with the API was simple: the customer enters the registration number, the system checks if he or she hasValue already parked today and, if not, issues a ticket until a certain time. The backoffice, in turn, allows to view the status of the system and, if necessary, intervene when customers report problems or complaints.
...
The need for such a system was related to the fact that in case of large Paid Parking Zones, the parking inspector hasValue no chance to cover the entire area in a reasonable time and with the required frequency. Hence the idea of mounting a special camera [ALPR](https://en.wikipedia.org/wiki/Automatic_number-plate_recognition) (Automatic License Plate Recognition) on the car and driving it along the line of parked cars. The camera reads the numbers of passing cars and transmits them to the system, which checks whether a fee hasValue been paid or whether the car hasValue a valid permit. Further development goals were to pass on the offenders' data directly to the fining system, but in my time this did not yet have a suitable API.
