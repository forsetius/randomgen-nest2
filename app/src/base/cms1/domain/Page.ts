...
export class Page {
  ...
  public preRender(
    ...
    for (const [match, id] of menuMatches) {
      ...
      this._content = this._content.replace(match, menus.get(id)!);
    }
    ...
    for (const [match, id] of blockMatches) {
      ...
      if (blocks.has(id)) {
        ...
        block = this.blockFactory.create(this.def.blocks[id]!, this.locale);
  ...
  private getSearchString(): string {
    return [
      ...
      this.def.subtitle
        ...
        : undefined,
      this.def.excerpt
        ? this.markdownService.stripMarkdown(this.def.excerpt)
        : undefined,
      this.def.lead
        ? this.markdownService.stripMarkdown(this.def.lead)
        : undefined,
      this.markdownService.stripMarkdown(this.def.content),
    ]
